...
do
    ...
    if [[ "${ARCH}" == "i386" || "${ARCH}" == "x86_64" ]]; then
        ...
        export LDFLAGS="-L${SSLPATH}/bin/${PLATFORM}${SDKVERSION}-${ARCH}.sdk/lib"
        export PKG_CONFIG_PATH="${SSLPATH}/bin/${PLATFORM}${SDKVERSION}-${ARCH}.sdk/lib/pkgconfig"
     echo "-----------------------???????当前目录PKG_CONFIG_PATH:${PKG_CONFIG_PATH}"
   
        mkdir -p "${CURRENTPATH}/bin/${PLATFORM}${SDKVERSION}-${ARCH}.sdk"
        LOG="${CURRENTPATH}/bin/${PLATFORM}${SDKVERSION}-${ARCH}.sdk/build-curl-${VERSION}.log"
        #./configure --with-zlib=${CURRENTPATH}/../zlib-1.2.11 --disable-shared --enable-static --host="${ARCH}-apple-darwin" --with-ssl=$SSLPATH --prefix="${CURRENTPATH}/bin/${PLATFORM}${SDKVERSION}-${ARCH}.sdk/" > "${LOG}" 2>&1
        ./configure --with-zlib=${CURRENTPATH}/../zlib-1.2.11 --disable-shared --enable-static --host="${ARCH}-apple-darwin" --with-ssl=$SSLPATH --prefix="${CURRENTPATH}/bin/${PLATFORM}${SDKVERSION}-${ARCH}.sdk/"
    else
        PLATFORM="iPhoneOS"
        SDKTYPE="iphoneos"
